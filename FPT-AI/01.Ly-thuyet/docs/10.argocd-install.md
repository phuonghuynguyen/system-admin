...
<h2 style="color:orange">8. add user argocd</h2>
Change context và namespace đến cụm k8s mong muốn, để kiểm tra đã có những user nào:

    # argocd account list
để add thêm user vào argocd, add config cm: argocd-cm:
![argocd-install19](../img/argocd-install19.png)<br>

    # kubectl edit cm argocd-cm
    add user huynp19
    data:
      accounts.huynp19: apiKey,login
![argocd-install20](../img/argocd-install20.png)<br>

<h3 style="color:orange">8.1. Change password user argocd</h3>

Exec vào pod argocd-server:
![argocd-install18](../img/argocd-install18.png)<br>

```
# kubectl exec --stdin --tty argocd-server-578fd59c4c-sc6tm -- /bin/bash
# argocd login localhost:8080 --insecure --username admin --password <admin_password>
# argocd account update-password --account huynp19 --new-password <password> --current-password <admin_password>
```
<h3 style="color:orange">8.2. Add permission user argocd</h3>

    # k edit cm argocd-rbac-cm
    thêm quyền all cho tab applications
![argocd-install21](../img/argocd-install21.png)<br>

Tham khảo chi tiết quyền của argocd: https://argo-cd.readthedocs.io/en/stable/operator-manual/rbac/